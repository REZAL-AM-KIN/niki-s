...
class RechargeSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def create(self, validated_data):
        try:
            consommateur = Consommateur.objects.get(pk=validated_data["cible_recharge"]["id"])
        except:
...
class BucquageSerializer(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        model = Bucquage
        fields = ("cible_bucquage", "nom_produit", "prix_produit", "entite_produit", "date")

    def create(self, validated_data):
        request = self.context.get("request")
        bucqueur = request.user
        try:
            consommateur = Consommateur.objects.get(pk=validated_data["cible_bucquage"]["id"])
        except:
            raise serializers.ValidationError("Cannot resolve cible id")
        try:
        ...
        if consommateur.activated == False:
        ...
        if bucqueur.groups.filter(name=produit.entite).exists() or bucqueur.is_superuser:
