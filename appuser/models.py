...
class Utilisateur(User):
    ...
    def entities(self):
        ...
        return Entity.objects.filter(
            models.Q(groups__in=self.groups.all()) | models.Q(groups_management__in=self.groups.all())).distinct()
...
if WITHLDAP:
    ...
    def updateLdapUser(sender, instance, **kwargs):
        if instance.is_active:
            if LdapUser.objects.filter(pk_django=instance.pk).count() == 1:  # si l'utilisateur existe, mise à jour
                ...
                ldapuser_to_save = LdapUser(full_name=instance.username, email=instance.email,
                                            first_name=instance.first_name, last_name=instance.last_name,
                                            password=instance.ldap_password, pk_django=instance.pk)
                ldapuser_to_save.save()
                manageLdapGroupMember("addmember", instance,
                                      instance.groups.all())  # on appelle la méthode pour gérer les groupes à ce niveau pour couvrir l'activation d'un utilisateur existant, précédemment désactivé, qui aurait des groupes
        else:
            if LdapUser.objects.filter(
                    pk_django=str(
                        instance.pk)).count() == 1:  # si l'utilisateur n'est pas actif, on le supprime du ldap
    ...
    def deleteLdapUser(sender, instance, **kwargs):
        if LdapUser.objects.filter(pk_django=instance.pk).count() == 1:
            ...
            # comme la suppression d'un utilisateur dans le ldap n'engendre pas automatiquement son appartenance aux
            # groupes, appel de la méthode de gestion des groupes
    ...
    class LdapGroup(ldapdb.models.Model):
        ...
        members = ListField(db_column='member', default=[
            'cn=admin,dc=rezal,dc=fr'])  # il est obligatoire d'avoir au moins un membre dans un groupe
        pk_django = CharField(
            db_column='description')  # utilisation de la pk django qu'on met dans le champ description
    ...
    # lorsque le champ Groupe d'un utilisateur est mis à jour, plusieurs signaux sont envoyé : post_add pour les ajouts,
    # post_delete pour les suppressions. On décide donc de faire un clear complet des groupes dans le ldap puis de faire
    # les mises à jour qui conviennent
